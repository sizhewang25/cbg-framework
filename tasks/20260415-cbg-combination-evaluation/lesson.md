...
### Evaluation Results
- **planar_circle multilateration outperforms spherical_circle**: For the same distance model, planar_circle polygon intersection (Paths B, C) consistently beats spherical_circle intersection (Path A). The degree-space polygon approximation captures the feasible region better than the vertex-only approach.
- **Why planar_circle wins**: spherical_circle (`circle_intersections` in helpers.py) computes exact great-circle pairwise crossing points, then filters to points inside all circles — the result is a **sparse set of boundary vertices**, not an area. planar_circle approximates each circle as a 100-point polygon and computes `reduce(intersection)` — the result is a **filled 2D polygon** representing the full feasible region. The vertex set from spherical_circle is biased toward edges and misses the interior shape, especially for thin/asymmetric intersections, leading to poor centroid estimates.
- **Boundary vertex mean on planar_circle > geometric centroid**: C1 (planar_circle+boundary mean, 333 km) slightly beats B1 (planar_circle+geom, 395 km). The area-weighted Shapely centroid doesn't always help — extracting boundary vertices and averaging can be more robust for skewed intersection polygons.
...
### Scope Decisions
- **Removed planar_annulus_weighted (C1) from evaluation**: User requested removal — Octant's fused Phase 2+3 needs separate validation before including in systematic comparison. Renamed D-path to C-path.
- **bounded_spline + redundant_circle uses outer radius only**: When annular constraints pass through legacy `circle_preprocessing()`, only `radius_km` (outer) is used. This is correct for the filtering step — the inner radius is only meaningful for planar_annulus or planar_annulus_weighted multilateration.
