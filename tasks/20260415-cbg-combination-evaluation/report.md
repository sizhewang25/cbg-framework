...
### 2026-04-24 — Multilateration Technique Comparison: `spherical_circle` vs `planar_circle` vs `planar_annulus`

Confirmed via code inspection (`scripts/framework/multilateration/`):

| Aspect | `spherical_circle` | `planar_circle` | `planar_annulus` |
|--------|-----------|---------|-------------------|
...
**`spherical_circle`** (`helpers.py:circle_intersections`): converts VP coordinates to 3D Cartesian, computes exact pairwise great-circle arc crossing points, keeps only points inside all circles. Returns a vertex list — no polygon area.

**`planar_circle`**: approximates each RTT circle as a 100-point polygon in degree space (with latitude compression correction: `km_per_deg_lon = 111 × cos(lat)`), then sequentially intersects all polygons. Returns an explicit Shapely area geometry.

**`planar_annulus`**: same polygon intersection as `planar_circle` but uses the full annular constraint — intersects outer disks then *subtracts* inner disks. Only meaningful with `bounded_spline` distance model which produces non-zero `inner_radius_km`. With disk-only constraints it degenerates to `planar_circle`.

**Key implication**: planar_circle and spherical_circle both discard `inner_radius_km`, so pairing them with `bounded_spline` wastes the annular constraint. Only `planar_annulus` (and `planar_annulus_weighted`) exploit the full Octant spline output.
