...
| Method | Source | Description |
|--------|--------|-------------|
| `spherical_circle` | Million-Scale CBG | `circle_intersections()` in `helpers.py:108` — exact spherical geometry: pairwise great-circle crossing points, filtered to points inside all circles. Returns vertex list |
| `planar_circle` | centroid_comparison.py | `_circle_to_planar_polygon()` + `reduce(intersection)` — approximates circles as 100-point polygons in degree space, computes Shapely polygon intersection. Returns Shapely geometry |
| `planar_annulus` | Octant | `compute_feasible_region_unweighted()` in `octant_geolocation.py:253` — intersect all outer disks, subtract all inner disks. Returns Shapely geometry |
| `planar_annulus_weighted` | Octant | `compute_feasible_region_weighted()` in `octant_geolocation.py:299` — grid-based weight accumulation (fused with Phase 2 filtering). Returns Shapely geometry |

Note: `planar_annulus_weighted` is an integrated Phase 2+3 — it does both filtering and region formation in one step.
...
- `intersection_weighted_filtering` is fused with `planar_annulus_weighted` (they are the same step)
...
Phase 3 output: one of:
    — vertex_list: List[(lat, lon)] from spherical_circle intersection
...
Phase 3 wrappers:
- `multilaterate_spherical_circle(circles)` → wraps `circle_intersections()` → vertex list
- `multilaterate_planar_circle(circles)` → Shapely polygon intersection → Shapely geometry
...
| # | Phase 1 | Phase 2 | Phase 3 | Phase 4 |
|---|---------|---------|---------|---------|
| B1 | speed_of_internet | circle_removal | planar_circle | geometric_centroid |
| B2 | low_envelope | circle_removal | planar_circle | geometric_centroid |
| B3 | bounded_spline | circle_removal | planar_circle | geometric_centroid |

**Path C: planar_annulus_weighted (fused Phase 2+3) → geometric_centroid**
...
**1. `planar_annulus` intersection (multilateration)**
...
- Different from existing `planar_circle` multilateration which uses `radius_km` (outer) only and ignores `inner_radius_km`
...
**Step 1: Add `planar_annulus` multilateration** (`scripts/framework/multilateration/planar_annulus.py`)
- Wraps `compute_feasible_region_unweighted()` from `octant_geolocation.py`
- Converts `CircleConstraint` → `AnnularConstraint` (same pattern as `planar_annulus_weighted.py`)
...
**Step 4: Validate**
- Compare `planar_annulus + geometric_centroid` against `octant_geolocation.py` standalone
