...
against `2**n - 1` regions, so past two sets the system is overdetermined and
some combination is always misdrawn. `EulerLayout.misplaced` measures exactly
how much and `plot` prints it, so the figure states its own error rather than
implying it has none.

Nothing here draws, and nothing here knows what a HEALPix ring is — this is
pure geometry over a boolean matrix. The ring-to-`top_n` mapping lives in
`membership.py` and the tolerance never reaches this module.

Ported from v3's `diagram/euler/layout.py` rather than imported: v4 does not
depend on v3, so that both layers' numbers can be recomputed independently from
their own trees. The geometry is unchanged; what differs is that empty sets are
dropped by the caller (`figure_euler.drop_empty_sets`) instead of aborting the
figure.
"""
...
#: `EULER_BOX`. 320 puts ~10 samples in a region holding 0.1% of the
#: population, which is the resolution the objective needs; the reported fit is
#: re-measured at `EULER_FIT_GRID`.
EULER_GRID = 320
...
#: swamped by any real area error and large enough to pick the compact layout
#: when two are equally faithful — which is every layout, once all the circles
#: that must be disjoint are.
COMPACTNESS = 1e-6

#: Restarts of the local search, the first from the MDS seed and the rest
#: jittered around it. Three because on the pooled runs restarts 4-6 never
...
def set_shares(membership: pd.DataFrame, order: list[str]) -> np.ndarray:
    """Share of the target population each method places within tolerance."""
...
def combination_shares(membership: pd.DataFrame, order: list[str]) -> np.ndarray:
    ...
    denominator, addressable by the bitmask arithmetic the layout needs. Index
    0 is the "no method correct" share, which lives outside every circle and is
...
def circle_radii(shares) -> np.ndarray:
...
def separation_for_overlap(r1: float, r2: float, target: float) -> float:
    ...
    the ones a reader is entitled to read straight off the picture:
    ...
    * otherwise a bisection on `lens_area`, which is monotone in `d`.
    ...
    # *above*, a perfectly nested pair falls through to a bisection whose
    # target the lens can never reach, and brentq rejects the bracket.
    #
...
def _area_sampler(n: int, radii: np.ndarray, grid: int):
    ...
    sit inside an optimizer loop. A fixed grid turns the whole partition into
    one `bincount` over per-point bitmasks: every point is charged to exactly
    the combination that contains it, so the areas are disjoint and sum to the
    union by construction — the same invariant `intersection_table` has.
    """
...
def fit_euler_layout(
    ...
    *,
    ...
    error over **every** combination, not just the pairs. Combinations observed
    empty are weighted up (`EULER_EMPTY_WEIGHT`) so the layout separates
    circles rather than inventing a region.

    Deterministic: the MDS start is a function of the data and the jitters are
    seeded by restart index, so the same membership matrix always yields the
    same figure.

    An **empty set has no circle** and is refused here rather than drawn at
    radius zero, which would be a dot the reader reads as "very small" instead
    of "never". Callers strip them with `figure_euler.drop_empty_sets` first —
    removing a set that no target belongs to changes no other region's count,
    so the diagram that results is the same one, minus a circle that could not
    be drawn.
    """
    from scipy.optimize import minimize

    n = len(order)
    if not 2 <= n <= len(SET_LETTERS):
        raise ValueError(f"an Euler layout takes 2-{len(SET_LETTERS)} sets, got {n}")
    if len(membership) == 0:
    ...
    if not (radii > 0).all():
        ...
        raise ValueError(
            f"{dead} placed no target within the tolerance, so there is no "
            f"circle to draw for them; strip empty sets with "
            f"`drop_empty_sets` or exclude them with --method"
        )
    ...
    def loss(flat) -> float:
        ...
        # below make that unreachable; the compactness term breaks the ties
        # that sent it there — with every pair already disjoint the objective
        # is exactly flat, and a flat plateau is where a direct search wanders.
...
def euler_fit_table(layout: EulerLayout, n_targets: int) -> pd.DataFrame:
    ...
    for key in range(1, 1 << n):
        ...
        rows.append(
            {
                "methods": "|".join(method_label(m) for m in members),
