...
//   node scripts/analysis/v4/tests/test_map_mtl_viewer.js <rendered.html>
//
// Forked from v3's harness. The architecture is kept -- stub the DOM and
// Plotly, eval the IIFE, drive every control, union the layers across draws --
// because the winding check it already carries is what guards the three NEW
// filled ring layers v4 adds. What is new here is `ringStructure` (the rings
// must be disjoint and within their HEALPix bounds), `verdictAgrees` (the
// drawn ring 0 must contain the prediction exactly when the status says
// `ring0`), and `retiredVocabulary` (no nearest-seed wording may survive in
// the prose the reader trusts).
...
const allLayerIds = ["showVoronoi", "showSeeds", "showNbhd", "showRings", "keptOnly",
...
// the fallbacks excluded -- `beyond` rows stay in, since they carry a real
// error and are exactly what a p95 lookup should land on. Both ends must select a real target and the
...
// ---- the ring neighbourhood ----------------------------------------------
//
// Bounds, never equalities: 24 cells at every nside sit on a corner of the
// 12-face base tessellation and have 7 neighbours rather than 8, so ring 1 can
// legitimately be short and ring 2 shorter.
statusSel.value = "all";
api.repopulate();
const payloadData = JSON.parse(payload);
const ringStructure = [];
for (let i = 0; i < nOpts; i++) {
  targetSel.value = String(i);
  api.redraw();
  const idx = api.layerIndex();
  const counts = [0, 1, 2].map((k) => {
    const tr = lastTraces[idx["ring" + k]];
    return tr ? splitRings(tr.lat, tr.lon).length : 0;
  });
  if (counts[0] !== 1) throw new Error(`ring 0 drew ${counts[0]} cells, not 1`);
  if (counts[1] > 8) throw new Error(`ring 1 drew ${counts[1]} cells, max is 8`);
  if (counts[2] > 16) throw new Error(`ring 2 drew ${counts[2]} cells, max is 16`);

  // Pairwise disjoint, keyed on the rounded first vertex. Ring 2 is a set
  // difference in Python; if it were not, ring 1's fill would draw over it and
  // the alpha would stack into a colour matching no legend swatch.
  const seen = new Map();
  for (let k = 0; k <= 2; k++) {
    const tr = lastTraces[idx["ring" + k]];
    if (!tr) continue;
    for (const ring of splitRings(tr.lat, tr.lon)) {
      const key = ring[0].map((v) => v.toFixed(3)).join(",");
      if (seen.has(key)) {
        throw new Error(`a cell is drawn in both ring ${seen.get(key)} and ring ${k}`);
      }
      seen.set(key, k);
    }
  }
  ringStructure.push(counts);
}

// ---- the verdict agrees with what is drawn -------------------------------
//
// The assertion with no v3 counterpart, and the one that catches a wrong ring
// layer: `status === "ring0"` must mean the prediction is inside the polygon
// the page shaded as ring 0. A drawing that disagrees with the verdict is
// worse than either being wrong on its own, because the reader believes the
// picture.
function pointInRing(pt, ring) {
  // Ray cast in (x=lon, y=lat). Ring vertices are [lat, lon].
  const x = pt[1], y = pt[0];
  let inside = false;
...
let verdictChecked = 0, verdictSkipped = 0;
for (let i = 0; i < nOpts; i++) {
  targetSel.value = String(i);
  api.redraw();
  const t = api.selected();
  if (!t.pred) continue;
  // Answered rows only. A FALLBACK carries a coordinate -- the Shortest-Ping
  // VP's, substituted by the benchmark -- and that coordinate can perfectly
  // well sit in the truth's own cell: tg-8b3bd95 on as01/vanilla_cbg lands
  // 1.55 km away, inside it, with `ring` 0. Its status is still `failed`,
  // because the METHOD declined to answer, and `classify.summarize` counts it
  // under `n_failed` rather than `n_ring0`. So containment says nothing about
  // the verdict there, and demanding agreement would assert that a fallback
  // which happens to land well is a success.
  if (t.status === "failed") { verdictSkipped++; continue; }
  const tr = lastTraces[api.layerIndex().ring0];
  if (!tr) continue;
  const ring = splitRings(tr.lat, tr.lon)[0];
  if (!ring) continue;
  // A cell straddling the antimeridian is emitted contiguous in longitude,
  // deliberately outside [-180, 180]; a planar ray cast is not meaningful
  // there. Never fires on CONUS data -- skipping keeps this from silently
  // becoming a test about the antimeridian.
  const lons = ring.map((p) => p[1]);
  if (Math.max(...lons) - Math.min(...lons) > 180) { verdictSkipped++; continue; }
  const inside = pointInRing(t.pred, ring);
  if (inside !== (t.status === "ring0")) {
    throw new Error(
      `target ${t.target_id}: status is "${t.status}" but the prediction is ` +
      `${inside ? "inside" : "outside"} the drawn ring-0 cell`
    );
  }
  verdictChecked++;
}

// ---- no retired vocabulary in the prose ----------------------------------
//
// Cheap, and it is the regression a careless port actually produces: a
// leftover `margin=—` in the meta strip, which reads as "this target has no
// margin" rather than "this concept is gone".
const RETIRED = /\bseeds? crossed\b|\bmargin\b|\btop-\d\b|\btop \d seeds?\b/i;
const retiredVocabulary = [];
targetSel.value = "0";
api.redraw();
for (const [where, text] of [["meta", el("meta").innerHTML], ...Object.entries(panels)]) {
  if (RETIRED.test(text)) retiredVocabulary.push(where);
}
if (retiredVocabulary.length) {
  throw new Error(
    `retired nearest-seed vocabulary survives in: ${retiredVocabulary.join(", ")}`
  );
}

const statuses = [...new Set(payloadData.targets.map((t) => t.status))].sort();
...
console.log(JSON.stringify({
  ...
  ringStructure, verdictChecked, verdictSkipped, statuses,
  statusOfSelected: api.selected().status,
}));
