...
Reads either:
- one cross-dataset YAML config (with a `datasets` list), or
- one or more cluster config YAML files,

and aggregates per-model classification accuracy for top-1 and top-3 from each
run's `cluster_scored` outputs.
...
- Accuracies are fractions in [0, 1] (not percentages).
...
def _resolve_dataset_configs_from_cross(cross_config: Path) -> list[Path]:
    cross_cfg = _load_yaml(cross_config)
    raw = cross_cfg.get("datasets")
    ...
    for idx, item in enumerate(raw):
        if not isinstance(item, str) or not item.strip():
            raise ValueError(f"datasets[{idx}] must be a non-empty string path")
        p = Path(item)
        if not p.is_absolute():
            p = (cross_config.parent / p).resolve()
        if not p.exists():
...
def _scored_dir_from_config(cfg: dict[str, Any]) -> Path:
...
def _inputs_dir_from_config(cfg: dict[str, Any]) -> Path:
...
def build_rows_from_cross_config(cross_config: Path) -> pd.DataFrame:
    cfg_paths = _resolve_dataset_configs_from_cross(cross_config.resolve())
    return build_rows(cfg_paths)
...
def main() -> None:
    ...
    group.add_argument(
        "--cross-config",
        type=Path,
        help="Cross-dataset YAML containing a 'datasets' list.",
    )
    group.add_argument(
        "--configs",
        type=Path,
        nargs="+",
        help="List of cluster config YAML files.",
    )
    parser.add_argument(
        "--out-csv",
        type=Path,
