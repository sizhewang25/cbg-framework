...
This module can also build the CSV directly from a cross-dataset YAML.
"""
...
from .build_topk_classification_benchmark_csv import build_rows_from_cross_config
...
def _run_id_to_cross_label_map(cross_config: Path) -> dict[str, str]:
    cross_cfg = _load_yaml(cross_config)
    datasets = cross_cfg.get("datasets")
    labels = cross_cfg.get("labels")

    if not isinstance(datasets, list) or not datasets:
        raise ValueError(f"'datasets' must be a non-empty list in {cross_config}")
    if not isinstance(labels, list) or len(labels) != len(datasets):
        raise ValueError(
            f"'labels' must be a list with same length as 'datasets' in {cross_config}"
        )

    mapping: dict[str, str] = {}
    for i, item in enumerate(datasets):
        if not isinstance(item, str) or not item.strip():
            raise ValueError(f"datasets[{i}] must be a non-empty string path")
        if not isinstance(labels[i], str) or not labels[i].strip():
            raise ValueError(f"labels[{i}] must be a non-empty string")

        cfg_path = Path(item)
        if not cfg_path.is_absolute():
            cfg_path = (cross_config.parent / cfg_path).resolve()
...
def _alias_run_id(run_id: str) -> str:
    ...
    if name.endswith("-weighted-final"):
        ...
        weighted = True
    elif name.endswith("-weighted-textbook"):
        name = name[: -len("-weighted-textbook")]
        weighted = True
    elif name.endswith("-final"):
        name = name[: -len("-final")]
    elif name.endswith("-textbook"):
        name = name[: -len("-textbook")]
...
def _build_wide_table(
    df: pd.DataFrame,
    run_id_display_map: dict[str, str] | None = None,
) -> pd.DataFrame:
    ...
    if run_id_display_map is None:
        out.index = [_alias_run_id(run_id) for run_id in out.index]
    else:
        out.index = [run_id_display_map.get(run_id, _alias_run_id(run_id)) for run_id in out.index]
...
def export_latex(
    ...
    wide = _build_wide_table(df, run_id_display_map=run_id_display_map)
...
def main() -> None:
    ...
    group.add_argument(
        "--cross-config",
        type=Path,
        help="Cross-dataset YAML (builds CSV first, then exports LaTeX).",
    )
    parser.add_argument(
        "--out-tex",
        type=Path,
    ...
    parser.add_argument(
        "--out-csv",
        type=Path,
        default=Path("scripts/analysis/outputs/cross_dataset_comparison02/classification/topk_classification_benchmark.csv"),
        help="Output CSV when using --cross-config.",
    )
    parser.add_argument(
        "--decimals",
        type=int,
    ...
    in_csv: Path
    run_id_display_map: dict[str, str] | None = None
    if args.cross_config is not None:
        table = build_rows_from_cross_config(args.cross_config)
        args.out_csv.parent.mkdir(parents=True, exist_ok=True)
        table.to_csv(args.out_csv, index=False)
        print(f"Saved {args.out_csv} ({len(table)} rows)")
        run_id_display_map = _run_id_to_cross_label_map(args.cross_config)
        in_csv = args.out_csv
    else:
        assert args.in_csv is not None
        in_csv = args.in_csv
    ...
    export_latex(
        ...
        run_id_display_map=run_id_display_map,
    )
