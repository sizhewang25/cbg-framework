...
def main() -> None:
    ...
    parser.add_argument("--top-n", type=int, default=1,
                        help="Top-N table to auto-load when --table is omitted. Default 1.")
    ...
    if table_path is None:
        candidates = [
            out_dir / f"{run_dir.name}_classification_table_top{args.top_n}.parquet",
            out_dir / f"{run_dir.name}_classification_table_top{args.top_n}.csv",
            out_dir / f"{run_dir.name}_classification_table.parquet",
            out_dir / f"{run_dir.name}_classification_table.csv",
        ]
        table_path = next((p for p in candidates if p.exists()), candidates[0])
    ...
    if combos is not None:
        keep = ["shortest_ping", *combos]
        keep_existing = [c for c in keep if c in table.columns]
        dropped = [c for c in table.columns if c not in keep_existing]
        missing = [c for c in keep if c not in table.columns]
        table = table[keep_existing]
        if missing:
            logger.warning("Requested combo columns missing from table: %s", ", ".join(missing))
        if dropped:
            logger.info("Dropped %d non-classification columns from table", len(dropped))
