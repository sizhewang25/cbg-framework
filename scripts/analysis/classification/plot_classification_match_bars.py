...
- Output CSV now also includes per-method status rates:
    success_rate, fallback_rate, error_rate (fractions in [0,1]).
...
def _parse_topn_combo(raw: str) -> tuple[int, int]:
    """Parse --topn-combo argument (e.g., '[1,3]') into (base_k, overlay_k) tuple."""
    try:
...
def _status_breakdown(df: pd.DataFrame) -> tuple[int, float, float, float]:
    """Return n_success and status percentages for one scored combo frame.

    Preferred source is explicit ``status`` values (SUCCESS/FALLBACK/ERROR).
    For backward compatibility with older scored CSVs (without ``status``),
    success is derived from the boolean ``success`` column and fallback/error
    percentages are reported as NaN.
    """
    n = len(df)
    if n == 0:
    ...
    return n_success, 100.0 * n_success / n, float("nan"), float("nan")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Default: auto-detect from top_n_values in config."
        ),
    )
    parser.add_argument(
        "--topn-combo",
        ...
        help=(
            ...
            "Default: auto-detect from top_n_values in config (use first 2 if available)."
        ),
    )
    ...
    # Auto-detect top_n_values and topn_combo from config if not provided
    top_n_values = cfg.get("top_n_values", [1])
    if args.topn_combo is None and len(top_n_values) >= 2:
        # Use first two values as overlay pair
        args.topn_combo = str([top_n_values[0], top_n_values[1]])
        logger.info("Auto-detected topn-combo from config: %s", args.topn_combo)

    # Auto-detect match_top_k from topn_combo if not provided
    if args.match_top_k is None:
        if args.topn_combo is not None:
            # Parse the combo and use the first value
            combo_pair = _parse_topn_combo(args.topn_combo)
            args.match_top_k = combo_pair[0]
        else:
            # Use the first top_n_value (or 1 if empty)
            args.match_top_k = top_n_values[0] if top_n_values else 1
        logger.info("Auto-detected match-top-k: %d", args.match_top_k)
    ...
    csv["success_rate"] = csv["success_pct"] / 100
    csv["fallback_rate"] = csv["fallback_pct"] / 100
    csv["error_rate"] = csv["error_pct"] / 100
    csv = csv[[
        "combo_id",
        "label",
        "n",
