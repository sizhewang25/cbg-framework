...
# ---- eval-bench-results → moved to scripts/analysis/cli.py -----------------
...
# ---- label-geo-for-targets (postprocessing) ---------------------------------

@app.command("label-geo-for-targets")
def cmd_label_geo_for_targets(
    run_id: str = typer.Option(..., help="Run id whose targets.parquet files to annotate."),
    outputs_root: Path = typer.Option(DEFAULT_OUTPUTS_ROOT, help="Root containing <run_id>/."),
) -> None:
    """Append target_continent + target_country to every targets.parquet (in place).

    Decoupled postprocessing: labels are reverse-geocoded
...
def cmd_materialize_target_space(
    ...
    # Only include completed combo dirs (run.json present) so partially written
    # outputs do not break downstream clustering/scoring.
    setup_dirs: dict[Path, list[Path]] = {}
    for tp in run_root.rglob("targets.parquet"):
        if not (tp.parent / "run.json").exists():
            continue
    ...
    for setup_dir, tpaths in sorted(setup_dirs.items()):
        ...
        # Geo labels are only required for geo-filtered answer spaces.
