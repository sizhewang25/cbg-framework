...
class TestNormalDistLTD(unittest.TestCase):
    ...
    def test_predict_extends_outer_at_finite_sentinel_slope_above_cutoff(self):
        ...
        # mu(rtt)=50*rtt, sigma=50, k=2 -> band=±100. cutoff_rtt=30 ->
        # inner(30)=1400, outer(30)=1600. With the default sentinel z at
        # (10_000 ms, 1_000_000 km), the extension slope is
        # (1_000_000 - 1600) / (10_000 - 30) ≈ 100.1404 km/ms, strictly
        # steeper than the asymptotic 100 km/ms.
        cutoff_rtt = 30.0
        sentinel_rtt = 10000.0
        outer_at_cutoff = 1600.0
    ...
    def test_predict_outer_above_cutoff_approaches_baseline_with_large_sentinel(self):
        ...
        self.assertTrue(result.success)
        # outer(30) = 1600; large-sentinel slope ≈ 1/THEORETICAL_SLOPE
        # → 1600 + 50/THEORETICAL_SLOPE = 6600.
        self.assertAlmostEqual(result.tg_distance.upper_km, 6600.0, places=2)
