"""Tests for the snapped geometric median centroid."""
...
from scripts.framework.centroid.geometric_median import GeometricMedianCentroid
...
class TestGeometricMedianCentroid(unittest.TestCase):
    def test_vertex_list_snaps_continuous_median_to_nearest_sample(self):
        centroid = GeometricMedianCentroid()

        point = centroid.select(
            successful_vertices([(0.0, 0.0), (0.0, 2.0), (0.0, 10.0)])
        )

        self.assertEqual(point, (0.0, 2.0))
    ...
    def test_region_sampling_returns_feasible_point(self):
        ...
        centroid = GeometricMedianCentroid(n_samples=8, seed=7)
    ...
    def test_multipolygon_region_sampling_returns_feasible_point(self):
        ...
        centroid = GeometricMedianCentroid(n_samples=32, seed=7)
    ...
    def test_zero_region_samples_falls_back_to_representative_point(self):
        region = rectangle_region()
        centroid = GeometricMedianCentroid(n_samples=0, seed=7)
