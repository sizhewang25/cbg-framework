...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Tolerance dividend.} We quantify the gap between coordinate-error and
    classification accuracy as the \emph{tolerance dividend}: in the proximity-sufficient
    operator regime, 53--68\% of all correct answers are won only by answer-space tolerance,
    invisible to any coordinate-error metric.

  \item \textbf{Failure taxonomy.} A two-layer geometric taxonomy---EMPTY\_REGION,
    EXCLUSIVE\_REGION, INCLUSIVE misclassification---partitions every wrong prediction by
    \emph{where} in the three-phase pipeline the error originates. In proximity-sufficient setups
    at $R = 50$\,km, EXCLUSIVE = 0: the bounded answer space absorbs containment failures,
    leaving centroid estimation (Phase 3) as the sole remaining failure mode for
    non-empty-intersection variants.
